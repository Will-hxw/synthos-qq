...
1. `BUG-002`、`BUG-003`、`BUG-005`：集中修正报告准确度、重试能力和群统计逻辑。
2. `BUG-008`：修正 Agent 取消/切换会话后仍继续执行的问题。
3. `BUG-010`：批量化 AI pipeline 中的串行查询和逐条写入。
4. `BUG-011`：提升媒体消息进入摘要和搜索的准确度。
5. `BUG-013`：优化前端主 chunk 体积和首屏加载体验。
